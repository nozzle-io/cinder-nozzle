...
- Package-shape check and zip output with exactly one top-level `cinder-nozzle/` folder.
...
| --- | --- | --- |
| CinderBlock package shape | PASS in CI | `tests/check_package.py` validates metadata, headers, sources, samples, CMake config, zip root |
| Host-independent C++ API build | PASS in CI | CMake builds static library and self-test on macOS/Linux/Windows |
| CPU pattern oracle 320x240 | PASS in CI | deterministic RGBA probes check no y-flip, no R/B swap, alpha bytes |
| CPU pattern oracle 641x479 | PASS in CI | odd-size deterministic RGBA probes check no y-flip, no R/B swap, alpha bytes |
